...
export default function SearchResultsPage() {
  ...
  const handleSearchParamChange = (param: string, value: string | number | Date | undefined) => {
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Adjust your search parameters here. Click save when you&apos;re done.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          id="check-in"
                          variant={"outline"}
                          className={`col-span-3 justify-start text-left font-normal ${!searchParams.checkIn && "text-muted-foreground"
                            }`}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          {searchParams.checkIn ? format(searchParams.checkIn, "PPP") : "Pick a date"}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0">
                        <Calendar
                          ...
                          onSelect={(date) => handleSearchParamChange('checkIn', date)}
                          initialFocus
                        />
                      </PopoverContent>
                    </Popover>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          id="check-out"
                          variant={"outline"}
                          className={`col-span-3 justify-start text-left font-normal ${!searchParams.checkOut && "text-muted-foreground"
                            }`}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          {searchParams.checkOut ? format(searchParams.checkOut, "PPP") : "Pick a date"}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0">
                        <Calendar
                          ...
                          onSelect={(date) => handleSearchParamChange('checkOut', date)}
                          initialFocus
                        />
                      </PopoverContent>
                    </Popover>
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleSearchParamChange('guests', parseInt(value))}
                    >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium" htmlFor="instant-book">Instant Book</label>
                  <Switch id="instant-book" />
                </div>
                <div>
                  <label className="text-sm font-medium" htmlFor="amenities">Amenities</label>
                  <div className="space-y-2 mt-2">
                    {['Wi-Fi', 'Kitchen', 'Air Conditioning', 'Washer', 'Free Parking'].map((amenity) => (
          ...
          <section className="w-full md:w-3/4">
            {mapView ? (
              <div className="bg-muted w-full h-[600px] rounded-lg flex items-center justify-center">
                <p className="text-muted-foreground">Map View (Integration required)</p>
              </div>
            ) : (
              <SearchResults searchTerm={searchTerm} setIsLoading={setIsLoading} />
            )}
